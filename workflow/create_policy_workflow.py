...
MAX_POLICY_GENERATION_ATTEMPTS = 3 # Maximum attempts for policy generation
...
class PolicyGenerationState(TypedDict):
    ...
    attempts_made: int # Number of times policy generation has been attempted
...
def generate_policy_node(state: PolicyGenerationState):
    ...
    attempts = state.get("attempts_made", 0) + 1 # Increment attempts for this generation
    print(f"  - Generation Attempt: {attempts} of {MAX_POLICY_GENERATION_ATTEMPTS}")
    ...
    generation_prompt = ChatPromptTemplate.from_template(
        ...
        **Instructions:**
        ...
        Generates a DirXML Script policy as an XML string based on a natural language question.
        The policy is created by a graph that deconstructs the question, identifies relevant XML tags,
        queries for their documentation, generates the policy, and validates it against a DTD.
        Args:
            question (str): The natural language request for the policy.
        Returns:
            str: The generated DirXML policy as a raw XML string.
        """
    )
    ...
    return {"generated_policy": policy.strip(), "validation_error": None,"attempts_made": attempts} # Store the updated attempt count
...
def validate_policy_node(state: PolicyGenerationState):
    ...
    try:
        ...
        root = etree.fromstring(xml_with_dtd.encode('utf-8'))
...
def should_regenerate(state: PolicyGenerationState):
    """
    Determines the next step after validation: regenerate, handle max retries, or end.
    """
    validation_err = state.get("validation_error")
    # attempts_made is the count *after* the last generate_policy call
    attempts = state.get("attempts_made", 0) 

    if validation_err:
        print(f"---VALIDATION FAILED (After attempt {attempts} of {MAX_POLICY_GENERATION_ATTEMPTS})---")
        if attempts >= MAX_POLICY_GENERATION_ATTEMPTS:
            print(f"---DECISION: Max attempts ({MAX_POLICY_GENERATION_ATTEMPTS}) reached. Routing to handle_max_retries.---")
            return "handle_max_retries" 
        else:
            # Note: The next attempt number will be `attempts + 1`.
            # `generate_policy_node` will increment `attempts_made` when it runs.
            print(f"---DECISION: Validation failed. Looping back to regenerate. (Next attempt will be {attempts + 1})---")
...
def build_policyCreate_graph():
    ...
    builder.add_conditional_edges(
        ...
        should_regenerate,
        {
            "generate_policy": "generate_policy", # If should_regenerate returns "generate_policy", go to this node
...
#     questions_to_test = [
#         # "if a user is active, add a description to them that says 'This account is currently active'",
#         "write a policy to add an attribute for users with attribute name dept=engg",
#         # "write a policy to check if the user's last name is missing then do veto "
#         # "write a policy to check if the user's first name is missing then do veto "
#     ]

#     # --- Loop through each question and run the graph ---
#     for i, question in enumerate(questions_to_test):
#         print(f"\n\n🚀🚀🚀--- Starting Test {i+1}: {question} ---🚀🚀🚀")

#         # Initialize the state for the current question
#         inputs = {
#             "question": question
#         }

#         final_state = None
#         # Stream through the graph execution to see intermediate states (optional)
#         # for s in policy_graph.stream(inputs, stream_mode="values"):
#         #     final_state = s
#         # Or just invoke to get the final state
#         final_state = policy_graph.invoke(inputs)

#         # --- Print the result for the current question ---
#         print(f"\n✅✅✅--- Final Policy for Test {i+1} ---✅✅✅")
#         if final_state.get("validation_error"):
#             print("Graph finished with an unresolvable validation error.")
#             print(f"Error: {final_state['validation_error']}")
#             print("\nLast attempted policy:")
#             print(final_state["generated_policy"])
#         else:
#             print(f"final response : {final_state["generated_policy"]}")
